...
class JsonCustomerRepo(CustomerRepo):
    ...
    def __store_to_file(self):
        ...
        file.close()
        if self._repo_type == Individual:
            company_list.append(json_file["company_list"])
            current_individual_index = self._id
            current_company_id = json_file["current_company_id"]
        if self._repo_type == Company:
            individual_list.append(json_file["individual_list"])
            current_individual_index = json_file["current_individual_index"]
            current_company_id = self._id
        json_list = {
            "current_company_id": current_company_id,
            "current_individual_index": current_individual_index,
    ...
    def __load_from_file(self):
        if self._repo_type == Individual:
            index = "current_individual_index"
